...
export const KnapsackSchema = z.object({
  ...
  maxIter: z.coerce
    .number({ invalid_type_error: 'Le nombre d’itérations doit être un nombre.' })
    .min(1, 'Le nombre d’itérations doit être supérieur à zéro.'),

  numParticles: z.coerce
    .number({ invalid_type_error: 'Le nombre de particules doit être un nombre.' })
...
export const MatrixSchema = z.object({
  ...
  matrices: z
    .array(z.array(z.number().min(0, 'Les distances doivent être positives ou nulles')))
    .refine(
      (matrix) => {
    ...
    .refine(
      (matrix) => {
        // Vérifie s'il y a au moins un nombre différent de zéro
        return matrix.some((row) => row.some((value) => value !== 0))
      },
      {
        message: 'La matrice ne peut pas contenir uniquement des zéros.',
      }
    ),
})
